<template>
  <div>
    <div class="committees">
      <div
        v-bind:key="`${committee.id}`"
        v-for="committee in committeesItems.reverse()"
        class="committee"
        :class="`accent-` + committee.name.toLowerCase()"
      >
        ...
        <div class="info-wrapper">
          ...
          <div class="info-set">
            ...
            <p>
              <vue-markdown>{{ committee.responsibilities }}</vue-markdown>
            </p>
          </div>
          <div class="info-set" v-if="committee.initiatives">
            <h6>Initiatives</h6>
            <p>
              <vue-markdown>{{ committee.initiatives }}</vue-markdown>
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    committeesItems() {
      return this.$static.committees.edges.map(({ node: committee }) => {
        return {
          id: committee.id,
          name: committee.name,
