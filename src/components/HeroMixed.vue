<template>
  <column class="hero-mixed">
    <block class="hero-mixed-layer-bottom">
      <div class="hero-mixed-side-left" v-bind:style="{ 'background-color': bgColor }"></div>
      <div class="hero-mixed-side-right">
        <img class="hero-mixed-side-right-img" v-if="bgImage" :src="bgImage" alt="" />
      </div>
    </block>
  ...
  export default {
    ...
    methods: {
      ...
      calcHeroMixedSizes() {
        ...
        let bottomLayerSideRightWidth = document.querySelector('.hero-mixed-side-right').offsetWidth;
        let bottomLayerSideRightHeight = document.querySelector('.hero-mixed-side-right').offsetHeight;
        let bottomLayerSideRightImg = document.querySelector('.hero-mixed-side-right-img');
        const heroMessageEl = document.querySelector(".hero-message");

        bottomLayerSideRightImg.setAttribute("style", `width: ${bottomLayerSideRightWidth}px; height: ${bottomLayerSideRightHeight}px`);
        
        // INITIAL VALUES
        let leftDistance = heroMessageEl.getBoundingClientRect().left;
        bottomLayer.setAttribute("style",
          `grid-template-columns: ${(leftDistance + (heroMessageEl.offsetWidth / 2))+'px'} 1fr;`);
